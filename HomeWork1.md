...
**`Удаленный репозиторий`** - это проект, который располагается не нам компьютера, а в сети интернете, либо на другом компьютере, к которому имеется доступ по средствам локальной сети, на сервере и тд.
...
Например, *репозиторий pull request* [Андрея Булгакова](https://github.com/AndreyBulgakov19/SCV_Git_1804).
